 ...
 * @param {string} bundleType - Bundle type: 'primitives' | 'color-semantics' | 'sizing-semantics' | 'utilities' | 'component' | 'component-utilities' | 'full' | 'full-utilities'
 * @param {string} customDescription - Custom description override
 * @param {string[]} additionalRequires - Additional dependencies to append to the requires list
 * @returns {string} Formatted header string
 */
function generateHeader(brand, bundleType, customDescription = null, additionalRequires = []) {
  ...
  const baseRequires = requiresMap[bundleType] || [];
  const allRequires = [...baseRequires, ...additionalRequires];
  ...
  const lines = [
    ...
    ...(allRequires.length > 0 ? ['', `Requires: ${allRequires.join(', ')}`] : []),
    '',
...
async function buildBrandComponents(brand) {
  ...
  for (const componentName of componentDirs.sort()) {
    ...
    if (typographyFiles.length > 0 || effectsClassFiles.length > 0) {
      let utilitiesContent = generateHeader(brand, 'component-utilities', `${componentName} Utility Classes`, [`components/${componentName.toLowerCase()}.css`]);
